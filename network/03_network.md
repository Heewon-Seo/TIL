...
![image](https://user-images.githubusercontent.com/87118337/147946073-99286bb8-5549-405d-a3af-40ecc8b6d916.png)
1. 컴퓨터 1에서 2로 연결 종료 요청(FIN)을 보낸다
2. 컴퓨터 2에서 1로 연결 종료 응답(ACK)을 반환
3. 컴퓨터 2에서 1로 연결 종료 요청(FIN)을 보냄
4. 컴퓨터 1에서 2로도 연결 종료 응답(ACK)을 반환

## 일련번호와 확인 응답 번호의 구조
![image](https://user-images.githubusercontent.com/87118337/147946474-e47ca7f4-bf23-4593-acfa-c0781149af67.png)
- 핸드셰이크가 끝나고 실제 데이터를 보내거나 상대방이 받을 때는 TCP 헤더의 3. 일련번호(sequence number)와 4. 확인 응답 번호(acknowledgement number)를 사용
- TCP는 데이터를 분할해서 보냄
- 일련번호: 송신 측에서 수신측에 이 데이터가 몇번째 데이터인지 알려주는 역할(전송된 데이터에 일련번호를 부여하면 수신자는 원래 데이터의 몇 번째 데이터를 받았는지 알 수 있음)
- 확인 응답 번호: 수신측이 몇번째 데이터를 수신했는지 송신 측에 알려줌
- 다음번호의 데이터를 요청하는데도 사용됨(10번 데이터를 수신하면 11번 데이터를 송신 측에 요청 > **확인 응답**)

![image](https://user-images.githubusercontent.com/87118337/147947742-704f57a0-1a00-4477-8213-f41ebc44666a.png)
- 데이터 전송하기 전 단계에서 3-way 핸드셰이크로 연결 수립이 될 때, 일련번호인 3001과 확인 응답 번호인 4001번이 결정됨
- 컴퓨터 1에서 2로 200바이트의 데이터 전송
- 컴퓨터 2는 200바이트 수신, 다음에 수신하고자 하는 데이터 번호를 확인 응답 번호에 넣는다(3001+200바이트 = 3201)
- 컴퓨터 1은 2로 3201부터 200바이트 데이터 전송
- 컴퓨터 2는 200바이트를 수신하고 다음에 수신하고자 하는 데이터 번호를 확인 응답 번호에 넣음 (3201+200바이트 = 3401)
- 이렇게 계속 반복
- 재전송 제어: 데이터가 항상 올바르게 전달되는 것은 아니므로 일련번호와 확인 응답 번호를 사용해서 데이터가 손상되거나 유실된 경우에 재전송하게 되어있음 
  - 데이터를 전송하는 도중에 오류가 발생하면 일정시간 동안 대기한 후 재전송 하는 것

### 윈도우 크기
- 이런 방식은 세그먼트(데이터)를 한개 보낼때마다 확인 응답을 한번 반환하므로 효율이 낮음
- 매번 확인 응답을 기다리는 대신 세그먼트를 연속으로 보내고 확인응답을 반환하는 식으로 효율을 높임
- 받은 세그먼트는 일시적으로 버퍼(buffer)에 보관함
- 받는 쪽에서 대량으로 데이터가 전송되어서 보관하지 못하고 넘쳐 버리는 것을 오버플로(overflow)라고 전송됨
- 오버플로가 발생하지 않도록 버퍼의 한계 크기를 알고 있어야 함 > 이게 바로 TCP 헤더의 윈도우 크기 값
- 윈도우 크기: 얼마나 많은 용량의 데이터를 저장해 둘 수 있는지를 나타내는 것
  - 확인 응답을 일일이 하지 않고 연속해서 송수신 할 수 있는 데이터 크기
  - 초깃값은 3-way 핸드셰이크를 할 때 판단

![image](https://user-images.githubusercontent.com/87118337/147948588-42592b31-0dcd-4106-9fef-4c53731a6f07.png)
![image](https://user-images.githubusercontent.com/87118337/147949291-2b804ce7-b5bb-408a-9551-1d459bae8bcc.png)

## 포트 번호의 구조
- 아까 언급했던 전송 계층의 역할 중에 목적지가 어떤 애플리케이션인지 구분하는 역할이 있었는데, 이를 구분하기 위해서 출발지 포트번호(source port number)와 목적지 포트번호(destination port number)가 필요함
- 포트번호는 0~65535번을 사용할 수 있음
  - 0~1023: 주요 프로토콜이 사용하도록 예약 > 이러한 포트를 well-known prots(잘 알려진 포트)라고 함
    - 서버 측 애플리케이션에서 사용
...
- 443: HTTPS
- 
![image](https://user-images.githubusercontent.com/87118337/147949895-83b3f468-43cd-47e1-8150-8266f747226f.png)
- 어떤 애플리케이션이 사용되고 있는지 구분하려면 TCP는 포트번호가 필요
- 웹 브라우저로 접속 할 때, 웹 브라우저에는 임의의 포트가 자동으로 할당 됨
- 그래서 서버측에서는 포트 번호를 정해둬야 하지만 클라이언트 측은 정하지 않아도 됨

## UDP의 구조
- UDP는 비연결형 통신이라서 확인 작업을 하지 않음
- 동영상 스트리밍 같은데 쓰임

### UDP 헤더
- UDP 헤더가 붙은 데이터 = UDP 데이터 그램
![image](https://user-images.githubusercontent.com/87118337/147950631-27af6c6f-7c6c-44fa-98cb-581be8c22309.png)
- 일일이 확인하지 않고 그냥 데이터를 보내기만 함

![image](https://user-images.githubusercontent.com/87118337/147950801-c44adf32-3812-476b-a132-76d066f3dcf5.png)
- UDP를 사용하면 랜에 있는 컴퓨터나 네트워크 장비에 데이터를 일괄로 보낼 수 있음 > **브로드캐스트**
