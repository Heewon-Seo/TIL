...
- 전이중 통신: 데이터의 송수신을 동시에 통신
  - 컴퓨터 간을 직접 랜 케이블로 연결 하는 방식(동시에 데이터 보내도 괜찮음)
- 반이중 통신: 회선 하나로 송신과 수신을 번갈아가면서 통신 (데이터를 동시에 전송하면 충돌 발생)
  - 허브(더미 허브)에 연결해서 사용하는 방식
- 허브가 아닌 스위치라면 충돌이 일어나지 않아서 전이중 통신 방식으로도 데이터를 주고받을 수 있음
- 최근에는 네트워크로 **스위치**를 사용하는 것이 표준 (허브 사용시에는 충돌이 생기면서 네트워크 지연이 발생하기 때문에)

### 충돌 도메인(collision domain)
- 충돌이 발생할 때 그 영향이 미치는 범위
- 스위치는 데이터를 동시에 송수신할 수 있어서 충돌이 일어나지 않고 충돌 도메인의 범위도 좁음
- 허브는 접속되어 있는 모든 컴퓨터가 충돌 도메인이 됨

## 이더넷의 종류와 특징
### 이더넷 규격
- 규격 이름의 뜻
![image](https://user-images.githubusercontent.com/87118337/147579459-6d59b4b2-1bba-47d0-a52d-3342f58b72d9.png)
- 통신속도: Mbps 단위인 통신 속도
- 전송방식: BASEBAND(펄스 신호에 의한 디지털 전송 방식)라는 전송 방식
- 케이블 종류
- 하이픈(-): 하이픈 뒤는 케이블 길이나 종류를 나타내지만, 동축케이블은 케이블의 최대 길이를 100미터 단위로 표시
  - 10BASE5는 케이블의 최대 길이가 500미터 이므로 5
  - 10BASE-T: UTP 케이블이라는 뜻
  - 최근 랜포트는 1000BASE-T가 일반적 10GBASE-T도 많아지고 있음
